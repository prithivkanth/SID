    ...
    , is_first_odom_(true)
{
    ...
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("walle_controller/odom", 10,
...
void OdometryMotionModel::odomCallback(const nav_msgs::msg::Odometry &odom)
{
    ...
    if(is_first_odom_)
    {
        ...
        // Initialize all samples to the current odometry pose
        for (auto &sample : samples_.poses)
        {
            sample.position.x = last_odom_x_;
            sample.position.y = last_odom_y_;
            tf2::Quaternion init_q;
            init_q.setRPY(0.0, 0.0, last_odom_theta_);
            sample.orientation.x = init_q.getX();
            sample.orientation.y = init_q.getY();
            sample.orientation.z = init_q.getZ();
            sample.orientation.w = init_q.getW();
    }
